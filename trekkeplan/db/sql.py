...
def sql_club_list(conn_mgr, raceid):
    ...
    cursor = conn.cursor()
    sql = """
SELECT cl.name Klasse, n.startnr 'St.nr' , n.name Navn, n.club Klubb, coalesce(cast(n.ecardno as char), '____') Brikke, coalesce(substring(cast(n.starttime as char),12,8), '____') Starttid
FROM names n
JOIN classes cl on cl.id = n.classid
...
JOIN races r on r.id = cl.raceid
WHERE r.id = %s
  AND (sb.name like %s OR sb.name is null)
  AND n.status not in ('V','X')
ORDER BY cl.sortorder, n.starttime is null, n.starttime, n.name COLLATE utf8mb3_danish_ci
"""
    try:
...
def sql_starter_list(conn_mgr, raceid, startlocation=None):
    ...
    cursor = conn.cursor()
    sql = """
SELECT n.startnr Startnr , n.name Navn, n.club Klubb, coalesce(cast(n.ecardno as char),'____') Brikke, cl.name Klasse
     , coalesce(substring(n.starttime,12,8), '____') Starttid
     ,'-' Startet
FROM names n
JOIN classes cl on cl.id = n.classid
LEFT JOIN svr_classstarts cls on cls.classid = n.classid
...
WHERE r.id = %s
  AND (sb.name like %s OR sb.name is null)
  AND n.status not in ('V','X')
ORDER BY n.starttime is null, n.starttime, cl.sortorder, n.name COLLATE utf8mb3_danish_ci
"""
    try:
        cursor.execute(sql, (raceid, f"{startlocation}%" if startlocation is not None else "%"))
        return cursor.fetchall(), [desc[0] for desc in cursor.description]
